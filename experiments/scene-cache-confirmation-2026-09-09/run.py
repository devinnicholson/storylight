"""Confirm dynamic versus bridged compiled decoding on 128 already-exposed inputs."""
...
CONFIRMATION_SHA = "1a2579008b49f64d2a2d45f00a2c4ed5c63c0621d1ca6accbf0637ff4f79bd69"
INPUT_SHA = "4ffc9adc3c15fbb6e684197a05b4b8bc2281f616ea02888f38989fff3acf58fb"
GENERATION_REFERENCE_SHA = "b308c7e236ef81b30915fe416639a6b8032e7a5faa2b94a422754082d62a4939"
CAPACITY = 1024
...
def plan(examples, mode):
    require(
        mode in MODES and len(examples) == 130 and len({row["id"] for row in examples}) == 130,
        "confirmation_inputs",
    )
    ...
    return [
        ...
        for n, i in enumerate([0, 1, *order])
    ]
...
def execute(args, helper, state):
    ...
    confirmation = helper.pinned_json(args.confirmation_dir / "protocol.json", CONFIRMATION_SHA)
    require(
        confirmation["model_manifest_sha256"] == args.model_manifest_sha256
        and confirmation["training_completed_sha256"] == args.completed_sha256
        and confirmation["bridge_sha256"] == BRIDGE_SHA
        and confirmation["generation_reference_sha256"] == GENERATION_REFERENCE_SHA
        and confirmation["input_sha256"] == INPUT_SHA
        and confirmation["max_runtime_seconds"] == args.max_runtime_seconds,
        "confirmation_binding",
    )
    rows = wrapper.read_rows(
        args.confirmation_dir,
        {"files": {"inputs.jsonl": {"sha256": INPUT_SHA, "rows": 130}}},
        "inputs.jsonl",
        130,
        helper,
        ["system", "user"],
    ...
    require(
        rows[:2] == [dict(id=r["id"], messages=r["messages"][:2]) for r in training[:2]],
        "fixed_training_warmups",
    )
    for row in rows:
        require(
            hashlib.sha256(row["messages"][0]["content"].encode()).hexdigest()
            == wrapper.PROMPT_SHA256,
            "prompt_changed",
        )
    require(
        {mode: plan(rows, mode) for mode in MODES} == confirmation["schedule"],
    ...
    helper.write_json(
        ...
        dict(
            ...
            examples=rows,
            preregistered_protocol_sha256=CONFIRMATION_SHA,
            input_sha256=INPUT_SHA,
            generation_reference_sha256=GENERATION_REFERENCE_SHA,
            calls=260,
            ...
            held_out=False,
            gold_read=False,
            scope="260 full generations on already-exposed inputs; no held-out quality claim",
            **runtime,
        ),
    )
    ...
    for i in range(2, 130):
        rows = {
            mode: next(
                r for r in records if r["case_index"] == i and r["mode"] == mode and not r["warmup"]
            )
            for mode in MODES
        }
        comparisons.append(
            dict(
                case_index=i,
                id=examples[i]["id"],
                identical_tokens=rows["dynamic"]["token_ids"]
                == rows["bridge-compile"]["token_ids"],
...
def main():
    ...
    parser.add_argument("--max-runtime-seconds", type=int, default=1600)
    ...
    args.wrapper.bounded(args, helper, execute, 1600)
