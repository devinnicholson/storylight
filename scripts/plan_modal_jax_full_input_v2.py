#!/usr/bin/env python3
"""Plan one immutable Modal v2 cached-input clone without remote mutation."""
...
from infra.gcp.jax.full_input_v2 import (  # noqa: E402
...
def _json_object(path: Path) -> dict[str, Any]:
    if path.is_symlink() or not path.is_file():
        raise ValueError(f"required v2 clone input is not a regular file: {path}")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise ValueError(f"required v2 clone input is not valid JSON: {path}") from error
    if not isinstance(value, dict):
        raise ValueError(f"required v2 clone input must be a JSON object: {path}")
    return value
...
def build_plan(
    *,
    ...
    return {
        "schema_version": "1.0",
        "producer": "bookforge-modal-jax-full-input-v2-clone-planner",
        "status": "plan-only",
        "source_run_id": source_run_id,
        "source_input_manifest_sha256": source_manifest_sha,
        "roundtrip_completion_sha256": completion_sha,
...
def main() -> None:
    ...
    if args.output.exists() or args.output.is_symlink():
        raise FileExistsError(f"refusing to overwrite v2 clone plan: {args.output}")
    plan = build_plan(
