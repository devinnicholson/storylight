...
def collect(
    *,
    ...
    if isinstance(runtime_detail, dict) and runtime_detail.get("data_dir"):
        checks["bookforge_storage"] = _command(
            ["findmnt", "-T", str(runtime_detail["data_dir"]), "-no", "SOURCE,TARGET"],
            required="nvme",
        )
    else:
        checks["bookforge_storage"] = {
            "ok": False,
    ...
    if service_pid is None:
        checks["privacy"] = {
            "ok": False,
            "detail": "A running Bookforge service PID is required for privacy evidence",
            "value": None,
        }
    else:
    ...
    if exercise_io:
        with tempfile.TemporaryDirectory(prefix="bookforge-hardware-") as directory:
            root = Path(directory)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Collect Bookforge Jetson hardware evidence")
    ...
    parser.add_argument(
        "--service-pid",
        type=int,
        help="running Bookforge service PID; readiness requires its privacy audit",
    )
