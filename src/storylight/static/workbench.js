...
function voiceTimingEvent(type, details = {}, epoch = recordingEpoch) {
  ...
  try {
    window.renderStorylightVoiceTiming?.(voiceTiming.events);
  } catch (_) {
...
async function fetchLiveSceneSession(emptySession = null) {
  ...
  try {
    ...
    if (response.status === 404) {
      const server = response.headers.get("X-Storylight-Server-Instance-Id");
...
function acceptedLiveScenePointer(response, snapshot) {
  const serverInstanceId = response.headers.get("X-Storylight-Server-Instance-Id");
  const sessionRevision = Number(response.headers.get("X-Storylight-Session-Revision"));
  if (
    !serverInstanceId
...
function renderPack(payload) {
  ...
  if (shouldPersist) {
    try {
      localStorage.setItem("storylight.latestStoryPack", JSON.stringify(pack));
...
if (!demoMode) {
  window.StorylightAnticipation.init({
...
window.storylightVoiceTiming = () => voiceTiming ? {
...
window.addEventListener("message", (event) => {
  if (event.origin !== window.location.origin
    ...
    || event.data?.type !== "storylight.preview-activated"
