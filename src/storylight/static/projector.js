...
const PROFILE_KEY = "storylight.projectionProfile.yaber-t1-pro";
const FIRED_CLASSES = ["action-reveal", "action-move", "action-transform", "action-open", "action-glow", "action-fade"];
const query = new URLSearchParams(window.location.search);
const SESSION_ID = query.get("session") || "storylight-live";
...
function queueLiveSceneSnapshot(envelope) {
  if (!LIVE_MODE || envelope?.type !== "storylight.live-scene") return;
  ...
  state.liveTransition = state.liveTransition.then(async () => {
    ...
    if (window.parent !== window) window.parent.postMessage({
      type: "storylight.preview-activated", sessionId: SESSION_ID, jobId,
  ...
  try {
    if (PACK_SOURCE === "latest") {
      ...
      if (response.ok) {
        ...
        const saved = localStorage.getItem("storylight.latestStoryPack");
        if (saved) {
    ...
    elements.startupError.textContent = `Storylight could not start: ${error.message}`;
    elements.startupError.classList.remove("hidden");
  }
}
...
updateProjection();
window.storylightHands = window.StorylightHands.init({
  ...
  screenToStage: (x, y) => window.StorylightHands.project(
