...
from storylight.visual_evaluation import MediaEvaluation, evaluate_media
from storylight.visual_lab import GPU_USD_PER_SECOND, GenerationRecord
...
class ModalSdkWarmInvoker:
    ...
    def __init__(
        self,
        *,
        app_name: str = "storylight-fast-scene",
    ...
    async def probe(self) -> tuple[bool, str]:
        try:
            ...
            return False, "Modal SDK is not installed; install storylight[modal-authoring]"
        try:
        ...
        *,
        ...
        if await self._should_generate_preview(request):
            preview_task = asyncio.create_task(
                ...
                name=f"storylight-live-preview-{job_id}",
            )
        resolved_task = asyncio.create_task(
            ...
            name=f"storylight-live-plan-and-renderer-{job_id}",
        )
        try:
        ...
        *,
        ...
        if prepare_renderer and (
            ...
            prepare_task = asyncio.create_task(
                ...
                name=f"storylight-renderer-preparation-{job_id}",
            )
            ...
            prepare_task = asyncio.create_task(
                ...
                name=f"storylight-provider-route-preparation-{job_id}",
            )
        try:
            resolved = await self._resolve_plan(
                request,
