"""OpenAI-compatible TensorRT client for Storylight's accepted four-slot protocol."""
...
from storylight.model_client import ModelUnavailableError, StructuredModelClient
from storylight.scene_facts import SceneFactsV2
from storylight.scene_prompt_routing import ROUTING_REVISION, scene_messages
...
def _source_text_from_plan_prompt(prompt: str) -> str:
    ...
    if marker not in prompt:
        raise ValueError("TensorRT slot planning requires a Storylight scene prompt")
    ...
    async def generate(
        ...
        *,
        ...
        if set(output_type.model_fields) != expected_fields:
            raise TypeError("TensorRT slot client only supports Storylight live-scene plans")
