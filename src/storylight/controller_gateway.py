...
The private Storylight API keeps listening on Jetson loopback. This process is the
...
_COOKIE_NAME = "storylight_controller"
...
class ControllerGatewaySettings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="STORYLIGHT_CONTROLLER_",
        extra="ignore",
    )
    ...
    public_name: Annotated[str, Field(min_length=1, max_length=80)] = "Storylight"
...
def create_controller_gateway(
    ...
    *,
    ...
    app = FastAPI(
        title="Storylight paired controller gateway",
        docs_url=None,
        redoc_url=None,
        openapi_url=None,
        lifespan=lifespan,
    ...
    async def pair_session(payload: PairingRequest) -> JSONResponse:
        ...
        response = JSONResponse(
            {"ready": True, "redirect": "/workbench?session=storylight-live"}
        )
        response.set_cookie(
            _COOKIE_NAME,
            session_id,
    ...
    async def proxy(path: str, request: Request):
        ...
        try:
            ...
            raise HTTPException(
                status_code=502,
                detail="Private Storylight API is unavailable",
            ) from error
