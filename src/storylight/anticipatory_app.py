"""Production assembly for the Storylight GKE anticipatory service."""
...
from storylight.anticipatory import AnticipatorySceneOrchestrator
...
from storylight.nemotron_critic import DEFAULT_NEMOTRON_VL_MODEL, NemotronVisionCritic
...
def build_runtime() -> AnticipatoryRuntime:
    renderer_url = _required("STORYLIGHT_RENDERER_URL")
    audience = os.environ.get("STORYLIGHT_RENDERER_AUDIENCE", renderer_url).strip()
    expected_gpu = os.environ.get("STORYLIGHT_RENDERER_GPU", "RTX_PRO_6000").strip()
    if expected_gpu not in {"L4", "RTX_PRO_6000"}:
        raise RuntimeError("STORYLIGHT_RENDERER_GPU must be L4 or RTX_PRO_6000")
    ...
    store = MemorySceneAssetStore(
        max_bytes=_integer(
            "STORYLIGHT_ANTICIPATORY_ASSET_BYTES",
            64 * 1024 * 1024,
    ...
    renderer = CloudRunAnticipatoryRenderer(
        ...
        timeout_seconds=_number(
            "STORYLIGHT_ANTICIPATORY_RENDER_TIMEOUT_SECONDS",
            30,
            minimum=1,
            maximum=120,
        ),
    ...
    nim_base_url = os.environ.get(
        "STORYLIGHT_NEMOTRON_BASE_URL",
        "http://127.0.0.1:8000",
    ).strip()
    nim = NemotronVisionCritic(
        base_url=nim_base_url,
        model=os.environ.get("STORYLIGHT_NEMOTRON_MODEL", DEFAULT_NEMOTRON_VL_MODEL),
        timeout_seconds=_number(
            "STORYLIGHT_NEMOTRON_TIMEOUT_SECONDS",
            30,
            minimum=1,
            maximum=120,
        ),
    ...
    orchestrator = AnticipatorySceneOrchestrator(
        ...
        max_concurrent_renders=_integer(
            "STORYLIGHT_ANTICIPATORY_MAX_RENDERS",
            2,
            minimum=1,
            maximum=2,
        ),
        max_candidates=_integer(
            "STORYLIGHT_ANTICIPATORY_MAX_CANDIDATES",
            64,
            minimum=2,
            maximum=256,
        ),
        cache_entries=_integer(
            "STORYLIGHT_ANTICIPATORY_CACHE_ENTRIES",
            32,
            minimum=0,
            maximum=128,
        ),
