...
from storylight.finite_modal_provider import _jpeg_dimensions
...
class LatencyClient:
    ...
    def headers(self) -> dict:
        ...
        key, secret = (
            values.get(f"STORYLIGHT_LATENCY_MODAL_{name}", "") for name in ("KEY", "SECRET")
        )
        if any(
    ...
    async def lookup(self, transport: str) -> None:
        ...
        if transport == "sdk" and self.instance is None:
            cls = self.modal.Cls.from_name("storylight-klein-latency-sdk", "LatencyStudio")
            await asyncio.wait_for(cls.hydrate.aio(), 10)
            self.instance = cls()
        elif transport == "http" and self.url is None:
            server = self.modal.Server.from_name("storylight-klein-latency-http", "LatencyServer")
