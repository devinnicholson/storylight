...
DEPTH_MODEL = "storylight-projection-depth-bootstrap"
...
class VertexGeminiImageSceneProvider:
    ...
    async def _probe_connection(self) -> tuple[bool, str]:
        ...
        try:
            ...
            # or a billable image. It does establish the same DNS/TLS/HTTP/1.1
        ...
        async with self._client_lock:
            ...
            if self._client is None:
                self._client = self._client_factory(
                    ...
                    # Requests are serialized; HTTP/2 multiplexing is unused.
                    # HTTP/1.1 avoids the observed closed-connection PING failure.
                    http2=False,
                    limits=httpx.Limits(
                        max_connections=1,
...
async def google_access_token() -> str:
    def fetch() -> str:
        try:
            ...
            raise VertexSceneProviderError(
                "Google authentication is unavailable; install storylight[gcp]"
            ) from error
...
def _request_payload(request: FastSceneRequest) -> dict[str, Any]:
    ...
    return {
        ...
        "generationConfig": {
            ...
            "candidateCount": 1,
            # Storylight seeds are uint32; Vertex's GenerationConfig uses int32.
