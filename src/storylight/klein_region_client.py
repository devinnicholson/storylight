...
from storylight.klein_latency_client import LatencyClient, LatencyError
...
class RegionClient(LatencyClient):
    ...
    async def lookup(self, transport: str) -> None:
        ...
        if transport == "sdk" and self.instance is None:
            cls = self.modal.Cls.from_name("storylight-klein-region-sdk", "RegionStudio")
            await asyncio.wait_for(cls.hydrate.aio(), 10)
            self.instance = cls()
        elif transport == "http" and self.url is None:
            server = self.modal.Server.from_name("storylight-klein-region-http", "RegionServer")
