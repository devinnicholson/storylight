...
from storylight.anticipatory import AnticipationStatus, BranchId, CommitRequest, SessionToken
...
from storylight.asr import LocalTranscriber, TranscriptionError, build_asr_backend
from storylight.asr_backend import AsrBackend, AsrBackendError, AsrBackendUnavailableError
...
from storylight.event_hub import (
    EventHubClosedError,
    PublishResult,
    ReaderEventHub,
...
from storylight.gcp_scene_provider import google_identity_token
...
from storylight.model_client import (
...
from storylight.runtime_status import RuntimeComponent, RuntimeStatus
from storylight.scene_foundry import SceneFoundry, SceneFoundryError
from storylight.service import StorylightService
from storylight.story_store import StoryPackCorruptError, StoryPackNotFoundError, StoryPackStore
...
async def lifespan(app: FastAPI):
    ...
    app.state.service = StorylightService(settings, client)
    ...
    if settings.live_scene_critic_backend == "nemotron":
        if not settings.live_scene_critic_url:
            raise ValueError("STORYLIGHT_LIVE_SCENE_CRITIC_URL is required for Nemotron")
        token_source = None
    ...
    async def find_completed_live_scene(payload: LiveSceneCreateRequest) -> StoryPack | None:
        from storylight.reviewed_description import (
            BOUNDED_REVISION,
            CURRENT_REVISIONS,
    ...
    if settings.live_scene_planner_auto_warmup:
        app.state.live_scene_planner_warmup_task = asyncio.create_task(
            ...
            name="storylight-live-planner-startup-warmup",
        )
    ...
    if settings.anticipatory_backend == "gke":
        if not settings.anticipatory_url:
            raise ValueError("STORYLIGHT_ANTICIPATORY_URL is required for the GKE backend")
        planner = getattr(app.state.live_scenes.provider, "planner", None)
        if planner is None or not callable(getattr(planner, "plan", None)):
            raise ValueError(
                "GKE anticipation requires STORYLIGHT_LIVE_SCENE_PLANNER=model"
            )
        token_source = None
...
async def runtime_status(request: Request) -> RuntimeStatus:
    ...
    service: StorylightService = request.app.state.service
    ...
    try:
        ...
        if payload.session_id is not None:
            ...
            if pointer.job.job_id == job.job_id:
                response.headers["X-Storylight-Server-Instance-Id"] = pointer.server_instance_id
                response.headers["X-Storylight-Session-Revision"] = str(pointer.session_revision)
        return job
...
async def preconnect_live_scene_provider(payload: dict[str, object], request: Request) -> dict:
    ...
    from storylight.provider_router import ResilientFastSceneProvider
...
async def prewarm_live_scene_provider(
    ...
    if payload.include_motion and not adapter.enable_motion:
        raise HTTPException(
            status_code=409,
            detail="Enable STORYLIGHT_LIVE_SCENE_ENABLE_MOTION before prewarming motion",
        )
    try:
...
async def prepare_live_scene_planner(
    ...
    if not payload.reviewed_description and (
        ...
        raise HTTPException(
            status_code=409,
            detail="STORYLIGHT_LIVE_SCENE_PLANNER is not configured as model",
        )
...
def _anticipatory_coordinator(request: Request) -> AnticipatoryEdgeCoordinator:
    ...
    if coordinator is None:
        raise HTTPException(
            status_code=409,
            detail="STORYLIGHT_ANTICIPATORY_BACKEND is disabled",
        )
    return coordinator
...
async def critique_live_scene(
    ...
    if critic is None:
        raise HTTPException(
            status_code=409,
            detail="STORYLIGHT_LIVE_SCENE_CRITIC_BACKEND is disabled",
        )
    registry: LiveSceneJobRegistry = request.app.state.live_scenes
    cache: AssetCache = request.app.state.asset_cache
    try:
...
async def live_scene_session_status(
    ...
    try:
        ...
        raise HTTPException(
            ...
            headers={"X-Storylight-Server-Instance-Id": registry.server_instance_id},
        ) from error
