...
from storylight.fidelity_lineage import stable_run_id
...
_STAGE_PRODUCERS = {
    "roundtrip": "storylight-jax-roundtrip-recorder",
    "train": "storylight-jax-training-recorder",
    "candidate-eval": "storylight-candidate-evaluation-recorder",
    "hf-export": "storylight-hf-release-recorder",
    "int4-export": "storylight-int4-export-recorder",
    "jetson-shadow": "storylight-jetson-shadow-recorder",
    "promotion": "storylight-trained-planner-promotion",
    "retain-baseline": "storylight-baseline-retention",
    "reconcile": "storylight-fidelity-reconciler",
}
...
STAGES = (
    ...
    StageSpec(
        "roundtrip",
        ("baseline",),
        "STORYLIGHT_JAX_ROUNDTRIP",
        "I_APPROVE_THIS_BOUNDED_ROUNDTRIP",
    ),
    StageSpec(
        "train",
        ("roundtrip",),
        "STORYLIGHT_JAX_TRAIN",
    ...
    StageSpec(
        "int4-export",
        ("hf-export",),
        "STORYLIGHT_JAX_INT4_EXPORT",
        "I_APPROVE_THIS_BOUNDED_GPU_EXPORT",
    ),
    StageSpec(
        "jetson-shadow",
        ("int4-export",),
        "STORYLIGHT_JAX_JETSON_SHADOW",
...
def _validated_stage_artifact(run: FidelityRun, stage_name: str, path: Path) -> dict[str, object]:
    ...
    if stage_name == "gate":
        if document.get("producer") != "storylight-fidelity-gate-builder":
...
def _validate_local_preflight_artifact(
    ...
    if document.get("producer") != "storylight-local-preflight":
...
def _validate_baseline_artifact(run: FidelityRun, document: dict[str, object]) -> None:
    if document.get("producer") != "storylight-baseline-recorder":
