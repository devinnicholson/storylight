...
class MfluxAssetGenerator(AssetGenerator):
    ...
    async def generate(self, request: ImageGenerationRequest) -> GeneratedImage:
        ...
        with tempfile.TemporaryDirectory(prefix="storylight-mflux-") as directory_name:
...
class MfluxDepthEstimator(DepthEstimator):
    ...
    async def estimate(self, image: GeneratedImage) -> GeneratedImage:
        ...
        if not ready:
            raise AssetGeneratorUnavailableError(detail)
        started = time.perf_counter()
        with tempfile.TemporaryDirectory(prefix="storylight-depth-") as directory_name:
...
class ModalAssetGenerator(AssetGenerator):
    ...
    async def generate(self, request: ImageGenerationRequest) -> GeneratedImage:
        ...
        if not ready:
            raise AssetGeneratorUnavailableError(detail)
        started = time.perf_counter()
        with tempfile.TemporaryDirectory(prefix="storylight-modal-") as directory_name:
