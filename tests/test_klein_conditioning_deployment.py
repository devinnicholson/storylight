...
def test_conditioning_deployment_admits_one_pinned_comparison(tmp_path, monkeypatch):
    ...
    class App:
        def __init__(self, name):
            assert name == "bookforge-klein-conditioning"
    ...
    source = root / "deploy/modal_klein_conditioning.py"
    ...
    assert mounts == [
        *((root / "deploy" / name, f"/root/{name}") for name in g["SOURCES"][:-1]),
        (
            root / "benchmarks/renderer-conditioning-2026-09-06/manifest.json",
            "/root/conditioning-manifest.json",
        ),
    ]
    frozen = json.loads(
    ...
    def fake_import(name, *args, **kwargs):
        if name in ("klein_conditioning_probe", "klein_scene_runtime"):
    ...
    with pytest.raises(RuntimeError, match="synthetic initialization failure"):
        compare()
    assert imports == ["klein_conditioning_probe", "klein_scene_runtime"]
    with pytest.raises(ValueError):
        compare()
