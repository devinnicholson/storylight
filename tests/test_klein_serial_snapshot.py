...
def audit(tmp_path, monkeypatch):
    ...
    source = root / "deploy/modal_klein_serial_snapshot.py"
    ...
    class App:
        def __init__(self, name):
            assert name == "bookforge-klein-serial-snapshot"
    ...
    packages["torch._inductor"] = SimpleNamespace(config=SimpleNamespace(compile_threads=1))
...
def test_serial_snapshot_verifies_capture_then_restores_without_model_reload(
    audit, monkeypatch, capsys
):
    a = audit
    assert {target for _, target in a.mounts} == {
        "/root/klein_scene_runtime.py",
        "/root/serial-snapshot-manifest.json",
    }
    ...
    events = [json.loads(line)["serial_snapshot_stage"] for line in logs.splitlines()]
    checks = [event for event in events if event["phase"] == "compiler_config"]
    assert len(checks) == 4
    assert all(event == {
        "phase": "compiler_config", "state": "checked",
        "original_env_state": "unset", "compile_threads": 1,
    } for event in checks)
    a.packages["torch._inductor"].config.compile_threads = 8
    with pytest.raises(ValueError, match="compiler configuration differs"):
        copy.deepcopy(captured).activate()


def test_serial_snapshot_refuses_expiry_bad_gpu_and_wrong_hash_without_retry(audit):
    a = audit
    a.clock["wall"] = 2001
...
def test_serial_snapshot_refuses_preinitialized_or_nonserial_compiler(audit, monkeypatch, capsys):
    a = audit
    monkeypatch.setenv("TORCHINDUCTOR_COMPILE_THREADS", "private-arbitrary-value")
    monkeypatch.setitem(sys.modules, "torch", object())
    with pytest.raises(RuntimeError, match="capture failed"):
        a.cls().capture()
    assert "capture:0" in a.claims and not a.calls
    assert os.environ["TORCHINDUCTOR_COMPILE_THREADS"] == "private-arbitrary-value"
    monkeypatch.delitem(sys.modules, "torch")
    # Independent admission simulates another experiment, never a retry of this claim.
    a.claims.clear()
    a.packages["torch._inductor"].config.compile_threads = 8
    with pytest.raises(RuntimeError, match="capture failed"):
        a.cls().capture()
    assert "capture:0" in a.claims and "runtime" not in a.calls
    assert os.environ["TORCHINDUCTOR_COMPILE_THREADS"] == "1"
    assert "private-arbitrary-value" not in capsys.readouterr().out
