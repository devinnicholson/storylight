...
import pytest
from test_klein_import_snapshot_benchmark import inputs as import_inputs
from test_klein_import_snapshot_benchmark import response as import_response

sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from scripts import benchmark_klein_warmed_snapshot as benchmark  # noqa: E402


def inputs(tmp_path, monkeypatch):
    manifest, content = import_inputs(tmp_path, monkeypatch)
    manifest.update(
        ...
        deployment_sha256=benchmark.legacy.file_hash(
            benchmark.ROOT / "deploy/modal_klein_warmed_snapshot.py"
        ),
        operations=[
            {"ordinal": i, "request_id": f"{i:032x}", "variant": "warmed_snapshot"}
            for i in range(3)
        ],
    )
...
def response(manifest, operation, content):
    value = import_response(manifest, operation, content)
    value["location"].update(cloud="CLOUD_PROVIDER_GCP", compute_region="us-central1")
    value["snapshot"].update(
        initialization={
            "model_load_seconds": 0.01,
            "cache_setup_seconds": 0.01,
            "warmup_seconds": 0.04,
        },
        warmups=[
            {
...
def test_fixed_proof_and_warmed_correctness_stay_separate_from_latency(tmp_path, monkeypatch):
    ...
    auth = {
        ...
        "reservation_id": "synthetic-warmed-snapshot",
        "reserved_usd": 1.15,
...
def test_capture_provenance_expiry_and_cancelled_submission_refuse_without_retries(
    ...
    async def cancellation():
        ...
        assert lookups == [("bookforge-klein-warmed-snapshot", "WarmedSnapshot")]
