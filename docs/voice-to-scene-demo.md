...
The local parser also handles “The English cream golden retriever ran down the cobblestone path in the city of Paris.” A bounded correction keeps the recognized breed phrase together when the small syntax model splits it into separate subjects. It preserves the breed description, action and path; Paris remains a disclosed local-only omission under the existing policy. The [repair evidence](../benchmarks/voice-retriever-2026-09-08/README.md) includes actual parser predictions, adversarial cases, API admission tests and live Jetson verification. This does not extend support to arbitrary compound phrases or override named-entity checks.

Simple noun-only descriptions such as “A red balloon” and “A Chicken Lollipop” are supported as still subjects, without inventing an action or setting. For these descriptions, **Finish recording** triggers generation; repeated partial nouns wait so that the rest of a sentence can arrive. Manual Generate also works. The [static-subject evidence](../benchmarks/voice-static-subject-2026-09-08/README.md) records the supported scope, privacy boundaries, tests and live checks. Reload the workbench after this update to receive the new partial-transcript guard.
