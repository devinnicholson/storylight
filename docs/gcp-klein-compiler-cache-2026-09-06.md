...
This **separate $6 maximum phase is incomplete**. L and M each completed ten requests; N's first request failed at the platform with an RTX quota error, and O was not run under the frozen stop rule. The first cache consumer was faster, but the balanced comparison did not complete and all 20 available paired images differed. There is no speed-screen pass or product promotion. The prospective protocol below remains unchanged; the partial result follows it.

Default model loading remains in place: the [eager-loading comparison](gcp-klein-loading-2026-09-06.md) failed its frozen speed and strict image-equivalence gates. Production routing, model weights, precision, compiler settings, prompts, seeds, and the watercolor target remain unchanged.
...
## Partial result and quota failure

There were **21 attempted POSTs: 20 successful results and one failed request**, yielding 40 verified JPEGs. L exported the verified 12,231,477-byte compiler artifact and 1,172-byte metadata; M restored those exact bytes. Both overlay builds succeeded and their returned layers, configuration, file hashes, and ownership were verified. The second consumer N produced no result, and the final baseline O remains unrun.

| Run | Completed requests | First client artifact-ready | Model-factory load | First 128-bucket image | First 256-bucket image | Later client median |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| L: baseline and producer | 10 | 99.263 s | 72.394 s | 17.648 s | 6.213 s | 0.470 s |
| M: cache consumer | 10 | 70.542 s | 49.085 s | 11.114 s | 1.586 s | 0.513 s |
| N: cache consumer | 0 | Failed request: 0.276 s | Unmeasured | Unmeasured | Unmeasured | Unmeasured |
| O: final baseline | Not run | Unmeasured | Unmeasured | Unmeasured | Unmeasured | Unmeasured |

M's cache-population call returned information after **0.286 seconds**. This confirms restoration was attempted successfully, not that particular compiler graphs hit the cache. Within the first pair, first-client time fell **28.93%**, and first-bucket image time fell **37.02% for 128** and **74.48% for 256**. Later worker medians were 0.348027 seconds for L and 0.352704 seconds for M. These are one-pair observations, not the frozen two-pair result; the model-factory times also differed substantially, so the client reduction cannot be attributed entirely to cache reuse. See the [L results](../benchmarks/gcp-klein-cache-2026-09-06/l/renders/summary.json) and [M results](../benchmarks/gcp-klein-cache-2026-09-06/m/renders/summary.json).

All **20 same-ordinal master/depth comparisons differed**, failing strict equivalence for the available pair. A separate nonblind assistant review found seven of eight unique scene cases satisfied the frozen facts in each arm. Both retained the extra silver fox and second lantern in the single-fox case; both retained watercolor appearance. Human review is pending. Visible similarity and diagnostic pixel measurements do not replace the failed byte gate or establish calibrated depth quality. [First-pair visual diagnostic](../benchmarks/gcp-klein-cache-2026-09-06/first-pair-visual-review.json).

N's first request received a platform 503 at **22:35:40 UTC**, explicitly naming the project's `nvidia_rtx_pro_6000_gpu_allocation_no_zonal_redundancy` quota as recently exceeded. Its journal records one failed request and zero completed cases. The evidence-collecting client exited zero and the supervisor recorded no control-process failure; those fields do not mean rendering succeeded. N's service deletion was verified after a 161.993-second supervised lifecycle. O was not substituted, retried, or sampled after the failure. [N results](../benchmarks/gcp-klein-cache-2026-09-06/n/renders/summary.json), [N platform logs](../benchmarks/gcp-klein-cache-2026-09-06/n/runtime-logs.json).

## Deletion and resource-release limits

The supervisor receipts verify **control-plane service deletion and observed absence**, not immediate GPU release or final charges. M's service deletion timestamp was **22:32:25 UTC**, but an unaligned Monitoring query later returned M's active-instance gauge as one through **22:38**, followed by zero at **22:39, 22:40, and 22:41**. The first observed zero was more than six minutes after deletion. The exact query records `aggregation: null`; these are sample timestamps, not merely late-arriving old timestamps. [Query](../benchmarks/gcp-klein-cache-2026-09-06/quota-diagnosis/refresh-1/query.json), [instance samples](../benchmarks/gcp-klein-cache-2026-09-06/quota-diagnosis/refresh-1/instances.json).

Cloud Monitoring samples this gauge every 60 seconds and may make data visible up to 120 seconds later. The observations do not establish the exact release instant, billable GPU duration, or that M caused N's quota rejection. The retained quota-allocation response has no RTX series; missing data is not zero usage. Google's ten-second shutdown window begins when SIGTERM is sent, not when a service deletion is requested. Do not use a successful delete command as an instantaneous GPU-availability signal. [Metric semantics](https://docs.cloud.google.com/monitoring/api/metrics_gcp_p_z), [service deletion](https://docs.cloud.google.com/run/docs/managing/services), [runtime shutdown contract](https://docs.cloud.google.com/run/docs/container-contract).

Billing reconciliation remains pending, and prior allowances remain retained. Neither the unrun O service nor the later zero gauge automatically releases funds or authorizes another attempt. A future comparison must account for resource-release evidence separately from control-plane deletion and preserve this incomplete phase as recorded.
