...
readonly ENGINE_DIR="${1:-${STORYLIGHT_EDGELLM_ENGINE_DIR:-}}"
...
if [[ ${EUID:-$(id -u)} -eq 0 ]]; then
  printf 'Run the TensorRT Edge-LLM server as the Storylight user, not root.\n' >&2
  exit 64
fi
...
# accidental LAN exposure while the candidate remains behind Storylight's gate.
