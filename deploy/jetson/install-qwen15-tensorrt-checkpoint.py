#!/usr/bin/env python3
"""Verify and atomically install the pinned Qwen 1.5B Edge-LLM checkpoint."""
...
EXPORT_ID = "qwen2.5-1.5b-instruct-awq-v010"
...
def verify_bundle(bundle_root: Path) -> tuple[dict[str, Any], list[Path]]:
    ...
    for required in (
        ...
        Path("llm/model.onnx"),
        Path("llm/model.onnx.data"),
        Path("llm/embedding.safetensors"),
    ):
...
def _default_destination() -> Path:
    ...
    return root / "models/qwen2.5-1.5b-instruct-awq-v010/onnx"
