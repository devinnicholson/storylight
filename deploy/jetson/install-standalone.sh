#!/usr/bin/env bash
# Install the already-staged Storylight checkout as a standalone Jetson service.

set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd -- "${SCRIPT_DIR}/../.." && pwd)"
TARGET_USER="${STORYLIGHT_SERVICE_USER:-${SUDO_USER:-${USER:-}}}"
DRY_RUN=0
DEPLOY_RENDERER=0
...
Installs Storylight's API and paired phone gateway services from /opt/storylight.
...
if [[ "$REPO_ROOT" != "/opt/storylight" ]]; then
  printf 'Run the staged installer at /opt/storylight/deploy/jetson/install-standalone.sh.\n' >&2
  exit 1
fi

readonly PYTHON=/opt/storylight/.venv/bin/python
if [[ ! -x "$PYTHON" ]]; then
  printf 'Missing Storylight virtual environment: %s\n' "$PYTHON" >&2
  exit 1
fi
"$PYTHON" -c 'import storylight.api, storylight.controller_gateway, modal'
...
install -d -o root -g root -m 0755 /etc/storylight
if [[ ! -e /etc/storylight/storylight.env ]]; then
  install -o root -g root -m 0600 \
    "$SCRIPT_DIR/storylight.standalone.env.example" /etc/storylight/storylight.env
  printf 'Created /etc/storylight/storylight.env; add Modal credentials before starting.\n'
fi

if [[ ! -e /etc/storylight/controller.env ]]; then
  pairing_token="$(openssl rand -hex 32)"
  if [[ ! "$pairing_token" =~ ^[a-f0-9]{64}$ ]]; then
    printf 'Pairing-token generation failed.\n' >&2
    exit 1
  fi
  install -o root -g root -m 0600 /dev/null /etc/storylight/controller.env
  {
    printf 'STORYLIGHT_CONTROLLER_BACKEND_URL=http://127.0.0.1:8080\n'
    printf 'STORYLIGHT_CONTROLLER_PAIRING_TOKEN=%s\n' "$pairing_token"
    printf 'STORYLIGHT_CONTROLLER_SESSION_TTL_SECONDS=43200\n'
    printf 'STORYLIGHT_CONTROLLER_MAXIMUM_SESSIONS=8\n'
    printf 'STORYLIGHT_CONTROLLER_COOKIE_SECURE=false\n'
    printf 'STORYLIGHT_CONTROLLER_PUBLIC_NAME=Storylight\n'
  } > /etc/storylight/controller.env
fi
...
if ((IMPORT_MODAL_PROFILE == 1)); then
  ...
  "$PYTHON" - "$modal_profile" /etc/storylight/storylight.env <<'PY'
import os
import re
import sys
import tempfile
...
if found != replacements.keys():
    raise SystemExit("The Storylight environment lacks Modal credential placeholders")

descriptor, temporary_name = tempfile.mkstemp(
    dir=environment_path.parent,
    prefix=".storylight.env.",
)
try:
...
install -o root -g root -m 0644 \
  "$SCRIPT_DIR/systemd/storylight@.service" /etc/systemd/system/storylight@.service
install -o root -g root -m 0644 \
  "$SCRIPT_DIR/systemd/storylight-controller@.service" \
  /etc/systemd/system/storylight-controller@.service
...
install -d -o "$TARGET_UID" -g "$TARGET_GID" -m 0700 \
  "$TARGET_HOME/.config/systemd/user" "$TARGET_HOME/.config/storylight"
install -o "$TARGET_UID" -g "$TARGET_GID" -m 0644 \
  "$SCRIPT_DIR/systemd/storylight-gemma.service" \
  "$TARGET_HOME/.config/systemd/user/storylight-gemma.service"
install -o "$TARGET_UID" -g "$TARGET_GID" -m 0644 \
  "$SCRIPT_DIR/systemd/storylight-tensorrt-planner.service" \
  "$TARGET_HOME/.config/systemd/user/storylight-tensorrt-planner.service"
install -o "$TARGET_UID" -g "$TARGET_GID" -m 0644 \
  "$SCRIPT_DIR/systemd/storylight-kiosk.service" \
  "$TARGET_HOME/.config/systemd/user/storylight-kiosk.service"
if [[ ! -e "$TARGET_HOME/.config/storylight/kiosk.env" ]]; then
  install -o "$TARGET_UID" -g "$TARGET_GID" -m 0600 \
    "$SCRIPT_DIR/kiosk.env.example" "$TARGET_HOME/.config/storylight/kiosk.env"
fi
install -d -o "$TARGET_UID" -g "$TARGET_GID" -m 0700 \
...
ln -sfn ../storylight-kiosk.service \
  "$TARGET_HOME/.config/systemd/user/default.target.wants/storylight-kiosk.service"
chown -h "$TARGET_UID:$TARGET_GID" \
  "$TARGET_HOME/.config/systemd/user/default.target.wants/storylight-gemma.service" \
  "$TARGET_HOME/.config/systemd/user/default.target.wants/storylight-kiosk.service"

systemctl daemon-reload
systemctl enable "storylight@${TARGET_USER}.service"
systemctl enable "storylight-controller@${TARGET_USER}.service"

modal_token_id="$(sed -n 's/^MODAL_TOKEN_ID=//p' /etc/storylight/storylight.env)"
modal_token_secret="$(sed -n 's/^MODAL_TOKEN_SECRET=//p' /etc/storylight/storylight.env)"
if [[ ${#modal_token_id} -lt 8 || ${#modal_token_secret} -lt 8 ]]; then
  printf 'Services enabled but not started: add Modal credentials to ' >&2
  printf '/etc/storylight/storylight.env, then start both units.\n' >&2
  exit 3
fi

if ((DEPLOY_RENDERER == 1)); then
  env MODAL_TOKEN_ID="$modal_token_id" MODAL_TOKEN_SECRET="$modal_token_secret" \
    /opt/storylight/.venv/bin/modal deploy /opt/storylight/deploy/modal_fast_scene.py
fi
...
systemctl restart "storylight-controller@${TARGET_USER}.service"
...
if ((standalone_ready == 0)); then
  printf 'Storylight API and controller did not become ready within 30 seconds.\n' >&2
  systemctl --no-pager --full status \
    "storylight@${TARGET_USER}.service" \
    "storylight-controller@${TARGET_USER}.service" >&2 || true
  exit 1
fi

printf 'Standalone Storylight is ready. Show the private pairing URL with:\n'
printf '  sudo /opt/storylight/deploy/jetson/show-controller-pairing.sh\n'
printf 'Gemma and the projector start after %s signs in graphically.\n' "$TARGET_USER"
printf 'Start or refresh the projector now with: systemctl --user restart storylight-kiosk.service\n'
printf 'For stable cable-free discovery, run once: sudo /opt/storylight/deploy/jetson/configure-portable-network.sh\n'
