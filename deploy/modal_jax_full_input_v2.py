"""Clone cached model bytes and a small v2 overlay into one training input prefix."""
...
APP_NAME = "bookforge-jax-full-input-v2-clone"
...
def _sha256(path: Path) -> str:
    if path.is_symlink() or not path.is_file():
        raise RuntimeError(f"required v2 clone input is not a regular file: {path}")
...
def _safe_relative(raw: str) -> Path:
    ...
    if pure.is_absolute() or ".." in pure.parts or pure.as_posix() != raw:
        raise RuntimeError("v2 clone declaration contains an unsafe path")
...
def _copy_verified(source: Path, destination: Path, row: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"v2 clone source is not a regular file: {source}") from error
    source_before = os.fstat(source_descriptor)
    if not stat.S_ISREG(source_before.st_mode) or source_before.st_size != row["bytes"]:
        os.close(source_descriptor)
        raise RuntimeError(f"v2 clone source size or type changed: {source}")
    ...
    if not stable or digest.hexdigest() != row["sha256"]:
        raise RuntimeError(f"v2 clone source changed while copying: {source}")
...
def _validate_request(
    ...
    if request.get("approval_token") != expected:
        raise ValueError("v2 full-training clone approval token is not exact")
    return (
        source_run_id,
        source_manifest_sha,
        completion_sha,
...
def clone_population(request: dict[str, object]) -> dict[str, object]:
    """Clone a complete v2 population without accepting cached bytes from the host."""
    ...
    partial = _INPUT_ROOT / f".full-input-v2-{target_run_id}.partial"
    ...
    if _sha256(overlay_manifest_path) != overlay_sha:
        raise RuntimeError("v2 overlay manifest checksum changed")
    ...
    if manifest_sha256(manifest) != target_manifest_sha:
        raise RuntimeError("derived v2 target manifest checksum changed")
    ...
    for row in base_manifest["files"]:
        ...
        _copy_verified(source, partial / _safe_relative(target_relative), rows[target_relative])
    ...
    if actual != set(rows):
        raise RuntimeError("v2 target population has undeclared or missing files")
    os.replace(partial, target)
    _write_manifest_last(target / "inputs.manifest.json", manifest)
    return {
        "schema_version": "1.0",
        "producer": "bookforge-modal-jax-full-input-clone-v2",
        "status": "staged",
        "source_run_id": source_run_id,
        "source_input_manifest_sha256": source_manifest_sha,
        "roundtrip_completion_sha256": completion_sha,
