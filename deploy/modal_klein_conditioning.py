"""One finite, isolated comparison of exact Klein text conditioning."""
...
MANIFEST = Path("/root/conditioning-manifest.json")
SOURCES = (
...
if modal.is_local():
    ...
    image = image.add_local_file(
        DEPLOY.parent / "benchmarks/renderer-conditioning-2026-09-06/manifest.json", str(MANIFEST)
    )
else:
    image = None

app = modal.App("bookforge-klein-conditioning")
...
def require(condition):
    if not condition:
        raise ValueError("conditioning comparison differs from its authorization")
...
def compare():
    ...
    print(json.dumps({"conditioning_initialization": "start"}), flush=True)
    from klein_conditioning_probe import run_comparison
    ...
    print(
        json.dumps(
            {
                "conditioning_initialization": "complete",
    ...
    records = run_comparison(runtime, value["cases"])
    return {
