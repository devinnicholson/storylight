"""Serial compiler mitigation for one verified warmed-runtime snapshot capture."""
...
APP_NAME = "bookforge-klein-serial-snapshot"
...
MANIFEST = Path("/root/serial-snapshot-manifest.json")
VERSIONS = {
    "torch": "2.8.0+cu128",
    "diffusers": "0.39.0",
    "transformers": "4.57.1",
...
if modal.is_local():
    image = (
        ...
        .add_local_file(
            DEPLOY.parent / "benchmarks/renderer-serial-snapshot-2026-09-05/manifest.json",
            str(MANIFEST),
        )
    )
else:
...
def _event(phase, state, **fields):
    print(
        json.dumps(
            {"serial_snapshot_stage": {"phase": phase, "state": state, **fields}}, sort_keys=True
        ),
        flush=True,
    )
...
class SerialSnapshot:
    ...
    def capture(self):
        ...
        try:
            if any(name == "torch" or name.startswith("torch.") for name in sys.modules):
                raise ValueError("compiler was initialized before serial configuration")
            previous = os.environ.get("TORCHINDUCTOR_COMPILE_THREADS")
            self.original_compiler_env = (
                "unset" if previous is None else "one" if previous == "1" else "other"
            )
    ...
    def _check_compiler(self):
        ...
        if type(config.compile_threads) is not int or config.compile_threads != 1:
            raise ValueError("serial compiler configuration differs")
        _event(
            "compiler_config",
            "checked",
            original_env_state=self.original_compiler_env,
            compile_threads=1,
        )
